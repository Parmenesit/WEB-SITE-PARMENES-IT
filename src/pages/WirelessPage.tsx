import { useEffect } from 'react';
import { ArrowLeft, Check, Wifi, Shield, Gauge, Users, Activity, Settings, MessageCircle } from 'lucide-react';
import { theme } from '../styles/theme';

interface WirelessPageProps {
  navigateTo: (page: string) => void;
}

const WirelessPage = ({ navigateTo }: WirelessPageProps) => {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);

  const features = [
    {
      icon: <Wifi className="w-8 h-8 text-[#A8CF45]" />,
      title: "Cobertura Total",
      description: "Eliminação de zonas de sombra e conexão estável em toda área."
    },
    {
      icon: <Shield className="w-8 h-8 text-[#A8CF45]" />,
      title: "Segurança WPA3",
      description: "Proteção avançada com as últimas tecnologias de criptografia."
    },
    {
      icon: <Gauge className="w-8 h-8 text-[#A8CF45]" />,
      title: "Alta Performance",
      description: "Suporte a Wi-Fi 6 e Wi-Fi 6E para máxima velocidade."
    },
    {
      icon: <Users className="w-8 h-8 text-[#A8CF45]" />,
      title: "Multi-Usuários",
      description: "Suporte eficiente para alta densidade de conexões."
    },
    {
      icon: <Activity className="w-8 h-8 text-[#A8CF45]" />,
      title: "Monitoramento Real-time",
      description: "Acompanhamento contínuo do desempenho da rede."
    },
    {
      icon: <Settings className="w-8 h-8 text-[#A8CF45]" />,
      title: "Escalabilidade",
      description: "Infraestrutura que cresce com seu negócio."
    }
  ];

  return (
    <div className="min-h-screen">
      {/* Hero Section */}
      <section className="relative h-[60vh] overflow-hidden">
        <div 
          className="absolute inset-0 bg-cover bg-center"
          style={{ 
            backgroundImage: 'url(https://images.pexels.com/photos/4800161/pexels-photo-4800161.jpeg)',
            filter: 'brightness(0.3)'
          }}
        />
        <div className="absolute inset-0 bg-gradient-to-b from-black/50 to-transparent" />
        <div className="relative container mx-auto px-4 h-full flex items-center">
          <div className="max-w-3xl">
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6 opacity-0 animate-[fadeInUp_1s_ease-out_forwards]">
              Projetos de Redes Sem Fio
            </h1>
            <p className="text-xl md:text-2xl text-white/90 mb-8 opacity-0 animate-[fadeInUp_1s_ease-out_0.3s_forwards]">
              Soluções Wi-Fi de alta performance para ambientes corporativos
            </p>
            <button 
              onClick={() => navigateTo('home')}
              className="inline-flex items-center text-white hover:text-[#A8CF45] transition-colors opacity-0 animate-[fadeInUp_1s_ease-out_0.6s_forwards]"
            >
              <ArrowLeft size={20} className="mr-2" />
              <span>Voltar para a página inicial</span>
            </button>
          </div>
        </div>
      </section>

      {/* Main Content */}
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto mb-16">
            <h2 className="text-3xl font-bold text-[#4B4B4D] mb-6 text-center">
              Conectividade Sem Fio de Alta Performance
            </h2>
            <p className="text-lg text-[#4B4B4D]/80 leading-relaxed">
              Em um mundo cada vez mais conectado, a qualidade da sua rede sem fio pode fazer toda a diferença 
              no desempenho dos negócios e na experiência dos usuários. A Parmenes IT é especialista em 
              desenvolver projetos de redes Wi-Fi de alta performance, alinhados com as mais modernas 
              tecnologias disponíveis no mercado.
            </p>
          </div>

          {/* Features Grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
            {features.map((feature, index) => (
              <div 
                key={feature.title}
                className="p-6 bg-white rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-100"
              >
                <div className="mb-4">{feature.icon}</div>
                <h3 className="text-xl font-semibold text-[#4B4B4D] mb-3">{feature.title}</h3>
                <p className="text-[#4B4B4D]/80">{feature.description}</p>
              </div>
            ))}
          </div>

          {/* Technology Section */}
          <div className="bg-gray-50 rounded-2xl p-8 mb-16">
            <h3 className="text-2xl font-bold text-[#4B4B4D] mb-6">Nossa Abordagem</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h4 className="text-xl font-semibold text-[#4B4B4D] mb-4">Análise Detalhada</h4>
                <p className="text-[#4B4B4D]/80">
                  Realizamos um mapeamento completo do ambiente, incluindo análise de interferências 
                  e estudo de cobertura para garantir o melhor desempenho.
                </p>
              </div>
              <div>
                <h4 className="text-xl font-semibold text-[#4B4B4D] mb-4">Tecnologia Avançada</h4>
                <p className="text-[#4B4B4D]/80">
                  Implementamos as mais recentes tecnologias Wi-Fi 6 e Wi-Fi 6E, garantindo 
                  maior velocidade e melhor desempenho.
                </p>
              </div>
              <div>
                <h4 className="text-xl font-semibold text-[#4B4B4D] mb-4">Segurança Robusta</h4>
                <p className="text-[#4B4B4D]/80">
                  Utilizamos protocolos avançados como WPA3 e implementamos políticas 
                  de segurança personalizadas.
                </p>
              </div>
              <div>
                <h4 className="text-xl font-semibold text-[#4B4B4D] mb-4">Monitoramento Contínuo</h4>
                <p className="text-[#4B4B4D]/80">
                  Oferecemos ferramentas de monitoramento em tempo real para garantir 
                  a qualidade do serviço.
                </p>
              </div>
            </div>
          </div>

          {/* Benefits Section */}
          <div className="bg-gray-50 rounded-2xl p-8 mb-16">
            <h3 className="text-2xl font-bold text-[#4B4B4D] mb-6">Benefícios para Seu Negócio</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                "Cobertura total sem zonas de sombra",
                "Alta performance mesmo com múltiplos usuários",
                "Segurança avançada com WPA3",
                "Monitoramento em tempo real",
                "Escalabilidade para crescimento futuro",
                "Suporte técnico especializado"
              ].map((item, index) => (
                <div key={index} className="flex items-start space-x-3">
                  <Check className="text-[#A8CF45] mt-1 flex-shrink-0" />
                  <span className="text-[#4B4B4D]">{item}</span>
                </div>
              ))}
            </div>
          </div>

          {/* Call to Action */}
          <div className="text-center">
            <h3 className="text-2xl font-bold text-[#4B4B4D] mb-4">
              Transforme sua conectividade sem fio
            </h3>
            <p className="text-lg text-[#4B4B4D]/80 mb-8">
              Entre em contato e descubra como podemos otimizar sua rede Wi-Fi!
            </p>
            <a
              href="https://api.whatsapp.com/send?phone=551132302721&text=Como%20Podemos%20ajudar%20Hoje?"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 glow-effect bg-[#A8CF45] hover:bg-[#8fb938] text-white px-8 py-4 rounded-lg font-medium transition-all duration-300 transform hover:scale-105 hover:shadow-lg group"
            >
              <MessageCircle className="w-5 h-5 transition-transform group-hover:rotate-12" />
              <span>Fale com um Especialista</span>
            </a>
          </div>
        </div>
      </section>
    </div>
  );
};

export default WirelessPage;